...
class Homegrid extends Component{
  ...
  componentDidMount() {
    ...
    if(newsSource == 'guardian'){
      ...
      var url = "http://localhost:3000/guardian/technology";
      ...
      console.log(newsSource);
      var url = "http://localhost:3000/nytimes/technology";
      fetch(url).then(response => response.json())
        .then(data =>
            this.setState({
              res: data.results,
        ...
        document.getElementById("technology").classList.add('activeButton');
        ...
        if(element != null){
          ...
          if(element.multimedia != null){
            ...
            for(var j = 0; j < assetlength; j++){
              if(element.multimedia[j].width > 1000){
                var img = element.multimedia[assetlength-1].url;
                break;
              }
            }
